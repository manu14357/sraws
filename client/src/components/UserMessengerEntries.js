            ...
            <Box sx={{ height: "100%" }}>
              <List sx={{ padding: 0, maxHeight: "100%", overflowY: "auto" }}>
                {props.conversations.map((conversation) => {
                  // Ensure conversation.recipient exists and has a valid username
                  const recipient = conversation.recipient || { username: 'Unknown User' };
                  return (
                    <UserMessengerEntry
                      conservant={props.conservant}
                      conversation={conversation}
                      key={recipient.username} // You can use a unique identifier like conversation._id instead if available
